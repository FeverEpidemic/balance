...
const lastUpdatedAt = new Date("2026-06-16T00:00:00+07:00");
const sectionKeys = [
  "scope",
  "generalPolicy",
  "exceptions",
  "nonRefundableSituations",
  "requestProcess",
  "requiredInformation",
  "reviewAndApproval",
  "processingTime",
    ...
    <main className="page-wrap py-8 md:py-12">
      <section className="glass-panel-strong mx-auto max-w-4xl rounded-[2rem] p-6 shadow-float md:p-10">
        <div className="max-w-3xl">
          <p className="eyebrow">{t("common.refundPolicy")}</p>
          <h1 className="headline-lg mt-4">{t("refundPolicy.title")}</h1>
          <p className="mt-3 text-sm text-muted-foreground md:text-base">
            {t("refundPolicy.lastUpdated", { date: formattedDate })}
          </p>
          ...
          {sectionKeys.map((sectionKey) => (
            <article key={sectionKey} className="card">
              <h2 className="headline-md">
                {t(`refundPolicy.sections.${sectionKey}.heading`)}
              </h2>
              <p className="mt-3 text-sm leading-7 text-muted-foreground md:text-base">
                {t(`refundPolicy.sections.${sectionKey}.body`)}
              </p>
            </article>
          ))}
        </div>
